...
public class ModularHelmet extends ArmorItem implements IModularArmor, IDEEquipment {
    private final TechLevel techLevel;

    public ModularHelmet(TechProperties props) {
        super(ArmorMaterials.DIAMOND, Type.HELMET, props);
        this.techLevel = props.getTechLevel();
    }

    @Override
    public boolean canEquip(ItemStack stack, EquipmentSlot armorType, Entity entity) {
        if (entity instanceof LivingEntity && !EquipmentManager.findItem(e -> e.getItem() instanceof ModularHelmet, (LivingEntity) entity).isEmpty()) {
            return false;
        }
    ...
    public boolean canEquip(ItemStack stack, LivingEntity livingEntity, String slotID) {
        if (!slotID.equals("head") || !EquipmentManager.findItem(e -> e.getItem() instanceof ModularHelmet, livingEntity).isEmpty()) {
            return false;
        }

        return !(livingEntity.getItemBySlot(EquipmentSlot.HEAD).getItem() instanceof ModularHelmet);
    }
    ...
    public ModuleHostImpl createHost(ItemStack stack) {
        ModuleHostImpl host = new ModuleHostImpl(techLevel, Math.round(ModuleCfg.chestpieceWidth(techLevel)/2), Math.round(ModuleCfg.chestpieceHeight(techLevel)/2), "helmet", ModuleCfg.removeInvalidModules);
