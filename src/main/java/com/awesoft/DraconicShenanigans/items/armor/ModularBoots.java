...
public class ModularBoots extends ArmorItem implements IModularArmor, IDEEquipment {
    private final TechLevel techLevel;

    public ModularBoots(TechProperties props) {
        super(ArmorMaterials.DIAMOND, Type.BOOTS, props);
        this.techLevel = props.getTechLevel();
    }

    @Override
    public boolean canEquip(ItemStack stack, EquipmentSlot armorType, Entity entity) {
        if (entity instanceof LivingEntity && !EquipmentManager.findItem(e -> e.getItem() instanceof ModularBoots, (LivingEntity) entity).isEmpty()) {
            return false;
        }
    ...
    public boolean canEquip(ItemStack stack, LivingEntity livingEntity, String slotID) {
        if (!slotID.equals("boot") || !EquipmentManager.findItem(e -> e.getItem() instanceof ModularBoots, livingEntity).isEmpty()) {
            return false;
        }

        return !(livingEntity.getItemBySlot(EquipmentSlot.FEET).getItem() instanceof ModularBoots);
    }
    ...
    public ModuleHostImpl createHost(ItemStack stack) {
        ModuleHostImpl host = new ModuleHostImpl(techLevel, Math.round(ModuleCfg.chestpieceWidth(techLevel)/2), Math.round(ModuleCfg.chestpieceHeight(techLevel)/2), "boots", ModuleCfg.removeInvalidModules);
